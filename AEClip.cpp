...
namespace AE
{
	...
	Clip::Clip(string chemin_feuille_sprites, int largeur_sprite, int hauteur_sprite, RGB transparent_color)
	{
		...
		Rect<int> rect_sprite(0, 0, largeur_sprite, hauteur_sprite); // largeur_sprite et hauteur_sprite est la même pour chaque sprite puisque cette fonction fait une copie de sprites égaux de taille
		...
		while( (rect_sprite.y+rect_sprite.h) <= (repeat_verti*rect_sprite.h) )
		{
			while( (rect_sprite.x+rect_sprite.w) <= (repeat_horiz*rect_sprite.w) )
			{
				...
				m_clip.push_back( Surface( Rect<int>(0, 0, rect_sprite.w, rect_sprite.h), (int)feuille_sprites.BitsPerPixel() ) ); // On ajoute au vector un objet Surface créé avec RGB_Surface
	...
	Surface Clip::to_feuille_sprites(Rect<int> dimensions, int depth) // Sauvegarde le clip dans une surface dont l'utilisateur indique la largeur et hauteur
	{
		if(dimensions.zero() )
		{
			for(int i = 0; i < m_clip.size(); i++)
		...
		for(position.y = 0; position.y < dimensions.h && i < m_clip.size(); position.y += m_clip[i].height() )
		{
			for(position.x = 0; position.x < dimensions.w && i < m_clip.size(); position.x += m_clip[i].width() )
			{
				...
				if( !m_clip[i].apply(to_return, Rect<int>(), position) )
					return Surface();
